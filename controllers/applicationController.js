...
validateApplication = async(req, next) => {
   ...
   if (!application){
       return next(new AppError("The application doesn't exist", 400))
   }
...
exports.apply = catchAsync(async (req, res, next) => {
    ...
    let application = await Application.find({employeeId: req.user._id, jobId: job._id});
    if (application.length > 0){
      return next(new AppError("There is already one application for this job", 400))
    }

    //create the application
    application = await Application.create({"employerId": job.employerId,
  ...
  exports.deleteApplication = catchAsync(async (req, res, next) => {
    ...
    await Job.update({_id: application.jobId}, {"nApplications": job.nApplications - 1});
    job = await Job.findById(application.jobId);
  ...
  exports.listAllApplication = catchAsync(async (req, res, next) => {
   ...
   exports.listJobApplications = catchAsync(async (req, res, next) => {
    ...
    const applications = await Application.find().where("jobId").equals(req.params.id)
    res.status(200).json({
        status: "success",
        data: {
          applications,
   ...
   exports.listEmployeeApplications = catchAsync(async (req, res, next) => {
    const userRole = req.user.role;
    if (userRole!="employee"){
        return next(new AppError("The user needs to be an employee to see their aplications", 400));
    }
    const applications = await Application.find().where("employeeId").equals(req.user._id)
    res.status(200).json({
        status: "success",
        data: {
          applications,
   ...
   exports.acceptApplications = catchAsync(async (req, res, next) => {
    ...
    await Application.update({_id: application._id}, {"status": "accepted"})
    ...
    await Job.update({_id: job._id}, {"status": status, "employeeId": employeeId});
   ...
   exports.declineApplications = catchAsync(async (req, res, next) => {
    //validation the application
    let application = await validateBeforeEditByEmployer(req, next);
    application.status = "declined"
    application = await Application.update(application)
